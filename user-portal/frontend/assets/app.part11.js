    ...
    (function () {
        try {
            ...
            var deferred = null;
            // Shared across showBanner + showIosInstructions so both localize.
            ...
            function showIosInstructions() {
                ...
                m.innerHTML = '<div style="background:#fff;color: var(--ink);border-radius:16px;max-width:360px;width:100%;padding:22px;text-align:center;">'
                    ...
                    + '<h3 style="margin:0 0 8px;font-size:18px;">' + A2T('pwa.iosTitle', 'Add to Home Screen') + '</h3>'
                    + '<p style="margin:0 0 6px;font-size:14px;color:var(--ink-soft);line-height:1.6;">' + A2T('pwa.iosStep1', '1. Tap the <strong>Share</strong> icon <span style="display:inline-block;">↑</span> in your browser bar') + '</p>'
                    + '<p style="margin:0 0 16px;font-size:14px;color:var(--ink-soft);line-height:1.6;">' + A2T('pwa.iosStep2', '2. Choose <strong>“Add to Home Screen”</strong>') + '</p>'
                    + '<button id="medxIosClose" style="background:var(--ink);color:#fff;border:none;border-radius:10px;padding:10px 22px;font-weight:600;cursor:pointer;">' + A2T('pwa.iosGotIt', 'Got it') + '</button>'
                    + '</div>';

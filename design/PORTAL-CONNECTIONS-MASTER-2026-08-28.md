...
| 7 | member/admin → Brevo | `sendEmail` U:75–145 (POST U:102) / A:71–135 (POST A:105); admin outbox drainer A:43350 | key read U:74/82/105, A:70/86/108 | **key absent ⇒ `[EMAIL DROPPED]` U:126–128, A:128–131; dumps to `EMAIL_DUMP_DIR` U:135–141** |
...
| Email | 58 admin outbox writers → `scheduled_emails` → `drainScheduledEmails` A:43350 → Brevo A:105; member direct sends U:75; member stages 10 row kinds (U:11383…) drained only by the admin in prod (dev drainer U:29516–29540) | `scheduled_emails` | **dropped today (no Brevo key)** |

...
function fileBlock(n, editing) {
  ...
  return `<div class="mx-note-files">
      ...
      ${docs.map(f => `<div class="mx-note-doc"><span>📎</span><a href="${esc(api.url(f.url))}" target="_blank" rel="noopener" title="${esc(f.name)}">${esc(f.name)}</a><span class="mx-hint">${sizeLabel(f.size)}</span>${editing ? `<span data-act="rmFile" data-id="${esc(f.id)}" role="button" class="mx-x" aria-label="Remove ${esc(f.name)}">×</span>` : ''}</div>`).join('')}

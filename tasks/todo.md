...
- [x] phase 2 (2026-09-22): member app `/live/:token?` (public, bare) + `/app/live` (member, 'user' token) — user-portal/frontend-v2/js/views/live.js + css/views/live.css: header · event switcher (mine + ALL EVENTS read-only) · NOW/NEXT strip · YOUR SLOTS (speakers) · PROGRAM (days → time blocks, kind chips, TBD placeholders, ONE ATTENDING toggle) · MY SCHEDULE (conflicts, .ics per session/day) · SPEAKERS (grid + sheet) · INFO (venue/map/dress/contact) · 60 s ?since= polling + "Program updated" toast · localStorage cache (live:program:<key>, live:attendance:<token>) · sw.js v2-4 precaches the view
- [x] entry points: OPEN THE EVENT APP → on /plexus/ticket + /gala/ticket (plexus-ticket.ticketPageHtml liveUrl) and /boston/me (kind bridges); "Your event app: <url>" under the wallet buttons in every ticket email (templates only, no re-sends); admin GET /api/live/speaker-link/:speakerId (program-ops, adminOnly); PLEXUS WEEK LIVE → on the Plexus tab strip, EVENT APP → on Building Bridges. Base = MEMBER_PORTAL_URL → PUBLIC_BASE_URL → RENDER_EXTERNAL_URL (plexus-ticket.liveAppBase)
- [x] tests/live-app.test.js (10 hermetic) + tests/live-app-smoke.py (Playwright; `--smoke=local` boots a scratch stack, `--smoke=staging` with LIVE_SMOKE_*)
- [ ] before the medx.hr flip: the production Express SPA fallback (server.js `app.get('*')`) 404s any path with an "extension" — `/live/<sig>.<kind>.<id>` has one — and serves the LEGACY frontend; the v2 shell must own `/live` on the production origin (Netlify's `/* /index.html 200` already does on staging)
- [ ] later: a 'guest' live-token kind for named guests (today a guest's email links the host's app — one schedule per party, like the QR); speaker "Slides: due/uploaded" line once the me payload carries it; push/Q&A per the brief's out-of-scope list
